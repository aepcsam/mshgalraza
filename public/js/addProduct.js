...
//checking user is loading in or not
window.onload = () => {
    if (user) {
...
let imagePaths = []; //will store all uploaded imaged paths
//
...
const validateForm = () => {
    if (!productName.value.length) {
        ...
        return showAlert(
            "enter few tags to help ranking your product in search"
        );
    } else if (!tac.checked) {
...
addProductBtn.addEventListener("click", () => {
    ...
    // Validate Form
    if (validateForm()) {
        //ValidationForm return true or false while doing validation
...
saveDraft.addEventListener("click", (req, res) => {
    // alert("draft");
    // store sizes
    storeSizes();
...
if (location.pathname != "/add-product") {
    ...
    //fetch the product data if product is not in search
